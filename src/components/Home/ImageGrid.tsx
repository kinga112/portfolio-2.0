...
import tires from "../../assets/image-grid/tires.jpg";
...
function ImageColumn(props: {
  ...
  const columnitems = props.images.map((image: string, index: number) => (
    <ColumnItem image={image} bgColor={props.bgColors[index]} />
  ));

  return (
    <>
      <div className="h-full flex-col inline-flex flex-nowrap group w-48 mx-2">
        <div className={`${style} ${props.animation}`}>{columnitems}</div>
        <div className={`${style} ${props.animation}`}>{columnitems}</div>
      </div>
    </>
  );
  ...
  return (
    <>
      <div className="pt-2">
        <div
          className={
            "w-full h-full min-w-44 rounded p-2 overflow-hidden object-contain " +
            props.bgColor
          }
        >
          <img
            className="object-contain rounded-sm hover:scale-150 duration-300"
            src={props.image}
          />
        </div>
      </div>
